...
const (
	...
	// HealthCheckShootCRDsManagedResourceAnnotationKey is a constant for an annotation on the ControlPlane resource
	// that indicates that the 'extension-controlplane-shoot-crds' ManagedResource is present.
	HealthCheckShootCRDsManagedResourceAnnotationKey = "provider.extensions.gardener.cloud/healthcheck-shoot-crds-managedresource"
)
...
func (a *actuator) reconcileControlPlane(
	...
	if a.controlPlaneShootCRDsChart != nil {
		...
		if !metav1.HasAnnotation(cp.ObjectMeta, HealthCheckShootCRDsManagedResourceAnnotationKey) {
			a.logger.Info("Adding annotation to ControlPlane", "controlplane", kutil.ObjectName(cp), "annotation", HealthCheckShootCRDsManagedResourceAnnotationKey)
			if err := extensionscontroller.AddAnnotation(ctx, a.client, cp, HealthCheckShootCRDsManagedResourceAnnotationKey, "true"); err != nil {
				return false, err
			}
		}
	}
