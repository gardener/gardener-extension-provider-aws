...
var _ = Describe("ConfigValidator", func() {
	...
	Describe("#Validate", func() {
		...
		It("should forbid VPC that doesn't exist", func() {
			awsClient.EXPECT().GetVPCAttribute(ctx, vpcID, string(ec2types.VpcAttributeNameEnableDnsSupport)).Return(false, &smithy.GenericAPIError{Code: "InvalidVpcID.NotFound"})
		...
		It("should forbid VPC that exists but has wrong attribute values or no attached internet gateway", func() {
			awsClient.EXPECT().GetVPCAttribute(ctx, vpcID, string(ec2types.VpcAttributeNameEnableDnsSupport)).Return(false, nil)
			awsClient.EXPECT().GetVPCAttribute(ctx, vpcID, string(ec2types.VpcAttributeNameEnableDnsHostnames)).Return(false, nil)
		...
		It("should allow VPC that exists and has correct attribute values and an attached internet gateway", func() {
			awsClient.EXPECT().GetVPCAttribute(ctx, vpcID, string(ec2types.VpcAttributeNameEnableDnsSupport)).Return(true, nil)
			awsClient.EXPECT().GetVPCAttribute(ctx, vpcID, string(ec2types.VpcAttributeNameEnableDnsHostnames)).Return(true, nil)
		...
		It("should fail with InternalError if getting VPC attributes failed", func() {
			awsClient.EXPECT().GetVPCAttribute(ctx, vpcID, string(ec2types.VpcAttributeNameEnableDnsSupport)).Return(false, errors.New("test"))
