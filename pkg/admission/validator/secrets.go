...
// Validate checks whether the given new secret is in use by Shoot with provider.type=aws
...
func (s *secret) Validate(ctx context.Context, newObj, oldObj client.Object) error {
	...
	isInUse, err := secretutil.IsSecretInUseByShoot(ctx, s.client, secret, aws.Type)
	if err != nil {
		return err
	}
