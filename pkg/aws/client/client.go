...
func (c *Client) GetVPCAttribute(ctx context.Context, vpcID string, attribute string) (bool, error) {
	vpcAttribute, err := c.EC2.DescribeVpcAttribute(
		ctx,
		&ec2.DescribeVpcAttributeInput{VpcId: &vpcID, Attribute: ec2types.VpcAttributeName(attribute)},
	)
	if err != nil {
		return false, err
	}
...
func (c *Client) UpdateSubnetAttributes(ctx context.Context, desired, current *Subnet) (bool, error) {
	...
	var privateDnsHostnameTypeOnLaunch ec2types.HostnameType
	if desired.PrivateDnsHostnameTypeOnLaunch == nil {
		privateDnsHostnameTypeOnLaunch = ec2types.HostnameTypeIpName
	} else {
		privateDnsHostnameTypeOnLaunch = ec2types.HostnameType(*desired.PrivateDnsHostnameTypeOnLaunch)
	}
